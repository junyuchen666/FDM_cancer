# -*- coding: utf-8 -*-
"""
Created on Tue Jul 18 16:02:50 2023

@author: ZhaoYaqi

Revision: 2023/09/19 by Junyu Chen
"""
...
if __name__ == "__main__":
    for item in os.listdir(path_1):  # Load all the images in the folder
        ...
        if maxLoc[0] > 150:
            ...
            if 2 < ret < 5 and area > 100:
                plt.imshow(src_ch1_2)
                plt.show()
                outpath_ch1 = output_dir_ch1 + '/' + str(item) + '_' + 'CH1.png'
                cv2.imwrite(outpath_ch1, src_ch1)

            if ret > 0 and area > 0:
                number_mat.append(ret)
                area_ch1_mat.append(area)

            median_src_ch0 = cv2.medianBlur(src_ch0, 3)
            # get the mask image
            thresh_ch0 = cv2.threshold(median_src_ch0, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
            # plt.imshow(thresh_ch0)
            # plt.show()
            mean_val_ch0 = cv2.mean(median_src_ch0, mask=thresh_ch1)

            val_ch0_mat.append(mean_val_ch0[0])
            val_ch1_mat.append(mean_val_ch1[0])

            print('ch0:', mean_val_ch0)
            print('ch1:', mean_val_ch1)

    dataframe = pd.DataFrame({'area': area_ch1_mat, 'number': number_mat})

    # Save DataFrame as csv
    # dataframe.to_csv("K562_exo/K562_exo_2/output_data.csv",index=False,sep=',')
    ...
    fig, ax = plt.subplots()
    plt.title("Sample1")
    plt.xlabel('max_intensity_ch0')
    plt.ylabel('max_intensity_ch1')
